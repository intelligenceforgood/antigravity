...
- **Knowledge Item Placement:** By default, agent instructions specify creating Knowledge Items (KIs) in the global `~/.gemini/antigravity/knowledge/` directory. However, for the `i4g` project, all KIs, workflows, and standards MUST be saved in the version-controlled `i4g/antigravity` repository (e.g., `i4g/antigravity/knowledge/standards/`). Always verify the project's native knowledge repository before using the global default.
